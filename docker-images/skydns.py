...
def putValue2SkyDNS(skyDNSIp,skyDNSPort,clusterId,nodeId,value):
    ...
    conn.request("PUT","/v2/keys/skydns/local/cluster/mysql-cluster/"+clusterId + "/" + nodeId+ "?" + "value=" + value ,value)
    ...
    if res.status == 200 :
        ...
        logger.error("skyDns ip %s:%d%s",skyDNSIp,skyDNSPort,"/v2/keys/skydns/local/cluster/mysql-cluster/"+clusterId + "/" + nodeId+ "?" + "value=" + value)
        logger.error("http res code:%d,res reason %s",res.status,res.reason)
    return 0

def getValueFromSkyDNS(skyDNSIp,skyDNSPort,clusterId):
    conn = httplib.HTTPConnection(skyDNSIp,skyDNSPort)
    conn.request("GET","/v2/keys/skydns/local/cluster/mysql-cluster/" + clusterId)
